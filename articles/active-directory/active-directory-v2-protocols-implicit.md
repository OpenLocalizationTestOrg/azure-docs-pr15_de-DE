...
> [AZURE.TIP] Versuchen Sie es kopieren und Einfügen der unterhalb der Anfrage in einer Browserregisterkarte! (Vergessen Sie nicht, ersetzen die `domain_hint` und der `login_hint` Werte durch die richtigen Werte für Ihre Benutzer)
...
| Nonce | Erforderlich | Einen Wert enthalten, in der Besprechungsanfrage, die von der app, die Bestandteil der resultierende Id_token als Anspruch generiert.  Die app kann dann diesen Wert, um die Wiedergabe token Angriffen zu verringern überprüfen.  Der Wert ist in der Regel eine zufällige, eindeutige Zeichenfolge, die verwendet werden kann, um den Ursprung der Anfrage zu identifizieren.  |
| Aufforderung | Erforderlich | Für die Aktualisierung und Token in einem Iframe ausgeblendete erste, sollten Sie `prompt=none` um sicherzustellen, dass der Iframe nicht auf der Anmeldeseite Version 2.0 abteilungsdrucker wird und sofort zurückgegeben. |
