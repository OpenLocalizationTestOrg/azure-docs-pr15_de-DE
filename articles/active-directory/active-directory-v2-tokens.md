...
Der Endpunkt Version 2.0 unterstützt das [OAuth 2.0 Autorisierungsprotokoll](active-directory-v2-protocols.md), welche sowohl Access_tokens als auch Refresh_tokens verwendet wird.  Authentifizierung und Anmeldung über [OpenID verbinden](active-directory-v2-protocols.md#openid-connect-sign-in-flow), einen dritten Typ von Token, die Id_token vorgestellt werden ebenfalls unterstützt.  Jede dieser Token wird als "Person"Token dargestellt.
...
Aktualisieren von Token Sicherheitstokens dem Ihre app neu erfassen können Zugriff auf Token in einen Fluss OAuth 2.0 sind.  Sie können Ihre app langfristiges Zugriff auf Ressourcen im Namen eines Benutzers zu erreichen, ohne Interaktion vom Benutzer.
...
Um eine Aktualisierung in einer token Antwort zu erhalten, Ihre app muss anfordern und gewährt werden die `offline_acesss` Bereich.   Weitere Informationen zu den `offline_access` Bereich, finden Sie [hier Zustimmung & Bereiche Artikel](active-directory-v2-scopes.md).
...
Details zu den erwarteten Werten für diese Ansprüche werden oben im [Abschnitt Id_token](#id_tokens)enthalten.
