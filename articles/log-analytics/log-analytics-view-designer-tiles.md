...
| Abfrage | Abfrage ausführen, um zu überprüfen, ob die Daten für die Ansicht verfügbar ist.  Wenn die Abfrage keine Ergebnisse zurückgibt, wird eine Meldung anstelle des Werts aus der Abfrage Hauptfenster angezeigt. |
| Nachricht | Meldung, die angezeigt werden, wenn die Abfrage den Datenfluss Überprüfung keine Daten zurückgibt.  Wenn Sie keine Nachricht bereitstellen, wird die *Bewertung Durchführung* angezeigt. |
| **Zeitintervall** |
...
| Vorgang | Der Vorgang auf die Value-Eigenschaft zu einem einzelnen Wert und ausführen.<br><br>-Summe: Fügen Sie die Werte aller Datensätze mit dem Eigenschaftswert hinzu.<br>-Prozentsatz: Prozentsatz der summierten Werte aus der Datensätze mit dem Eigenschaftswert auf die summierten Werte aller Datensätze verglichen. |
...
| **Zeitintervall** |
| Dauer | Dauer ab dem aktuellen Datum für den Zeitraum aus der Abfrage verwendet werden soll.  Beispielsweise ist **7 Tage** angegeben ist, klicken Sie dann die Abfrage auf Einträge erstellt von 7 Tagen auf das aktuelle Datum beschränkt. |
| Ende Datenoffset | Optionaler Offset aus den aktuellen Daten für den Zeitraum aus dem Hauptfenster Abfrage verwendet werden soll.  Wenn **-1-Tag** für das **Ende Datum Offset** und **7 Tage** für die **Dauer**verwendet verwendet wird, beträgt beispielsweise klicken Sie dann die Abfrage auf Einträge erstellt von 8 Tagen zu gestern beschränkt. |
