...
2014 eingeführt Azure Ressourcenmanager, die des Konzepts einer Ressourcengruppe hinzugefügt. Eine Ressourcengruppe ist eines Containers für Ressourcen, die einen allgemeinen Lebenszyklus gemeinsam nutzen. Das Modell zur Bereitstellung von Ressourcenmanager bietet mehrere Vorteile:
...
- Eines erforderlichen Speicher-Kontos, das die virtuellen Festplatten für einen virtuellen Computer, einschließlich des Betriebssystems, temporäre und weiteren Daten Datenträger (Speicher) gespeichert.
- Eine optionale virtuelle Netzwerk, der als einen zusätzlichen Container, fungiert, in dem Sie eine Subnetz Struktur zu erstellen und bestimmen das Subnetz auf dem des virtuellen Computers befindet (Netzwerk).
...
Die folgende Tabelle beschreibt die Änderungen in Datenverarbeitung, Netzwerk und Speicher Ressourcenanbieter Interaktion:
 ...
 ---|---|---
| Cloud-Dienst für virtuellen Computern |  Cloud-Dienst wurde eines Containers zum Halten von den virtuellen Computern, die Verfügbarkeit von der Plattform und den Lastenausgleich erforderlich. | Cloud-Dienst ist nicht mehr ein Objekt für das Erstellen eines virtuellen Computers mit dem neuen Modell erforderlich ist. |
| Virtuelle Netzwerke | Ein virtuelles Netzwerk ist optional des virtuellen Computers. Wenn enthalten ist, kann das virtuelle Netzwerk mit Ressourcenmanager bereitgestellt werden. | Virtuellen Computer erfordert ein virtuelles Netzwerk aus, das mit den Ressourcen-Manager bereitgestellt wurde. |
...
|Virtuelle IP-Adresse | Cloud Services erhalten eine Standard-VIP (virtuelle IP-Adresse) aus, wenn ein virtuellen Computers auf einen Clouddienst hinzugefügt wurde. Die virtuelle IP-Adresse ist die Adresse, den Lastenausgleich implizit zugeordnet.    | Öffentliche IP-Adresse ist eine Ressource, die den Microsoft.Network-Anbieter bereitgestellt werden. Öffentliche IP-Adresse kann es sich um statische (reserviert) oder dynamische sein. Ein Lastenausgleich kann dynamische öffentlichen IP-Adressen zugewiesen werden. Öffentliche IP-Adressen kann mithilfe von Sicherheitsgruppen gesichert werden. |
...
|Öffentliche IP-Adresse (PIP) pro virtueller Computer | Öffentliche IP-Adressen können auch direkt eines virtuellen Computers zugeordnet werden. | Öffentliche IP-Adresse ist eine Ressource, die von den Microsoft.Network-Anbieter verfügbar gemacht werden. Öffentliche IP-Adresse kann statische (reserviert) oder Dynamic. Jedoch können nur dynamische öffentlichen IP-Adressen zu Netzwerk-Schnittstellen zugeordnet werden, können Sie eine öffentliche IP-Adresse pro virtueller Computer sofort zu gelangen. |
