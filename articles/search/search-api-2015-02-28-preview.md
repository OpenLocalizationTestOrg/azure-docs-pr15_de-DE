...
Beachten Sie, dass Sie die Antwort auf nur die Eigenschaften filtern können, die Sie interessiert. Angenommen, wenn Sie nur eine Liste der Indexnamen, verwenden den OData `$select` Option Abfrage:
...
> [AZURE.NOTE] Dokument Tasten können nur Buchstaben, Zahlen, Striche enthalten ("-"), Unterstriche ("_") und Gleichheitszeichen ("="). Weitere Einzelheiten hierzu finden Sie unter [Naming Regeln](https://msdn.microsoft.com/library/azure/dn857353.aspx).
...
HTTPS ist erforderlich für Serviceanfragen. **Die Suchabfrage** kann mithilfe der Methoden GET oder POST erstellt werden.
...
3)  Mithilfe eines Filters, der vorherigen facettierten Abfrageergebnisse einschränken, nachdem der Benutzer auf klickt Bewertung 3 "und" Kategorie "Motels":
    ...
    ERSTE /indexes/hotels/docs? suchen = Test & Vertriebsstrategie = Tags & Vertriebsstrategie = BaseRate, Werte: 80 | 150 | 220 & $filter = Bewertung Eq 3 und Kategorie Eq 'Motels' und api-Version = 2015-02-28-Vorschau
...
6) Suchen Sie den Index in mehreren Feldern. Beispielsweise können speichern und Abfragen in mehreren Sprachen, alle im gleichen Index durchsuchbare Felder.  Englisch und Französisch Beschreibungen gemeinsame im selben Dokument vorhanden ist, können einige oder alle in den Abfrageergebnissen zurückgegeben werden:
...
Beachten Sie, dass Sie jeweils nur ein Index Abfragen können. Erstellen Sie mehrere Indizes für die einzelnen Sprachen nicht, es sei denn, Sie beabsichtigen, einzeln Abfragen.
...
9)  Abrufen einer bestimmten Folge von Felder:
...
11) Suchen Sie die Index und zurück Fragmenten mit Treffer wichtigsten Informationen
...
12) Suchen Sie den Index und Dokumente von näher an Entfernung von einen Verweis Speicherort sortiert zurück
    ...
    ERSTE /indexes/hotels/docs? suchen etwas = & $orderby=geo.distance (Speicherort geography'POINT(-122.12315 47.88121)') & api-Version = 2015-02-28-Vorschau
...
13) Suchen den Index, vorausgesetzt, es gibt ein Bewertungssystem Profil namens "Geo" mit zwei Abstand Punktzahl Funktionen, eine Definieren eines Parameters namens "CurrentLocation" und eine definieren einen Parameter namens "LastLocation"
    ...
    ERSTE /indexes/hotels/docs? suchen etwas = & ScoringProfile = Geo & ScoringParameter = CurrentLocation – 122.123,44.77233 & ScoringParameter = LastLocation – 121.499,44.2113 und api-Version = 2015-02-28-Vorschau
...
HTTPS ist erforderlich für Serviceanfragen. Die Anfrage **Nachschlagen Dokument** kann wie folgt erstellt werden.
...
`$select=[string]`(optional) – eine Liste der durch Trennzeichen getrennte Felder zum Abrufen. Wenn nicht angegeben, oder legen Sie auf `*`, sind alle Felder, die im Schema als abrufbaren markiert in der Projektion enthalten.
...
Nachschlagen des Dokuments, die Taste "2" enthält
...
HTTPS ist erforderlich für Serviceanfragen. Die **Anzahl von Dokumenten** Anforderung kann mithilfe der GET-Methode erstellt werden.
...
- `api-key`: Die `api-key` wird verwendet, um die Anfrage Suchdienst authentifizieren. Es ist ein Zeichenfolgenwert mit Ihrem Dienst-URL aus. Die **Anzahl von Dokumenten** Anforderung kann angeben, entweder einen Administrator oder Abfrage Taste für `api-key`.
...
Textkörper Antwort enthält den Count-Wert als ganze Zahl im nur-Text formatiert.
...
Bei der Verwendung von HTTP-GET **Vorschläge** API aufrufen, müssen Sie beachten, dass die Länge der URL der Anforderung 8 KB nicht überschreiten kann. Dies ist normalerweise genug für die meisten Applikationen. Einige Applikationen Naturprodukte jedoch sehr große Abfragen, insbesondere OData Filterausdrücke. In diesem Bereich ist die unter Verwendung von HTTP POST besser geeignet, da größere Filter als abrufen können. Mit bereitstellen wird die Anzahl der Klauseln in einem Filter die Beschränkung nicht die Größe der Filterzeichenfolge unformatierten, da das Größenlimit Anfrage für Beitrag ungefähr 16 MB ist.

> [AZURE.NOTE] Obwohl das Größenlimit der Beitrag Anforderung sehr groß ist, können nicht Filterausdrücke beliebig komplex werden. Weitere Informationen zum Filtern Komplexität Einschränkungen finden Sie unter [Ausdruckssyntax OData](https://msdn.microsoft.com/library/dn798921.aspx) .

**Anfordern**

HTTPS ist erforderlich für Serviceanfragen. Die Anfrage **Vorschläge** kann mithilfe der Methoden GET oder POST erstellt werden.
...
`highlightPreTag=[string]`(optional) – eine Zeichenfolge markieren, die zum Suchen von Treffer voran. Eingestellt werden `highlightPostTag`.
...
`highlightPostTag=[string]`eine Zeichenfolge (optional) – zu kennzeichnen, fügt um Treffer zu suchen. Eingestellt werden `highlightPreTag`.
...
`minimumCoverage`(optional, Standardeinstellungen auf 80)-eine Zahl zwischen 0 und 100, der angibt, des Prozentsatzes der der Index, die von einer Abfrage Vorschläge für die Abfrage aus, damit Sie als einen Erfolg gemeldet Reihenfolge verdeckt werden muss. Standardmäßig muss mindestens 80 % des Indexes verfügbar sein oder `Suggest` HTTP-Statuscode 503 zurückgegeben werden kann. Wenn Sie festlegen `minimumCoverage` und `Suggest` hergestellt wurde, es wird HTTP 200 zurückgeben und enthalten eine `@search.coverage` Wert in der Antwort, die Angabe des Prozentsatzes der der Index, die in die Abfrage eingeschlossen wurde.
