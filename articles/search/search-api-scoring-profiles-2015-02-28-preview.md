...
**Notiz** Eine Punktzahl Funktion kann nur auf Felder angewendet werden, die gefiltert werden.
...
| `boost` | Für die Bewertung der Funktionen erforderlich ist. Eine positive Zahl, die als Multiplikator für unformatierten Punktzahl verwendet. Es kann nicht gleich 1 sein. |
| `fieldName` | Für die Bewertung der Funktionen erforderlich ist. Eine Punktzahl-Funktion kann nur angewendet werden auf die Felder, die Teil des Indexes die feldauflistung, aufweisen und gefiltert. Darüber hinaus führt jede Funktionstyp zusätzliche Einschränkungen (Aktualität wird mit Datetime-Felder, Ausmaß mit ganze Zahl oder doppelte Felder, Abstand mit Feldern Speicherort und Tag mit Zeichenfolge oder Zeichenfolge Websitesammlung Felder verwendet). Sie können nur ein einzelnes Feld pro Funktionsdefinition angeben. Beispielsweise um Ausmaß zweimal in demselben Profil verwenden zu können, müssen Sie zwei Definitionen Ausmaß erhöhen, einen für jedes Feld aufnehmen möchten. |
| `interpolation` | Zur Bewertung der Funktionen erforderlich. Definiert die Steigung für die der Punktzahl verstärken erhöht ab dem Anfang des Bereichs bis zum Ende des Bereichs. Gültige Werte sind `linear` (Standardeinstellung), `constant`, `quadratic`, und `logarithmic`. Weitere Informationen finden Sie unter [Interpolationen festlegen](#bkmk_interpolation) . |
...
| `magnitude:constantBoostBeyondRange` | Gültige Werte sind True oder False (Standard). Bei der Einstellung true, die vollständige steigern weiterhin auf Dokumente anzuwenden, die einen Wert für das Zielfeld aufweisen, die über das obere Ende des Bereichs liegt. Mit FALSCH belegt, wird nicht die steigern dieser Funktion auf Dokumente, die einen Wert für das Zielfeld, das außerhalb des Bereichs liegen Probleme angewendet werden. |
