...
Visual Studio können Sie um das Anwendungspaket zu erstellen, das mehrere Gast-Programme enthält. [Verwenden von Visual Studio Verpacken eine vorhandene Anwendung](service-fabric-deploy-existing-app.md#using-visual-studio-to-package-an-existing-executable)wird angezeigt. Nachdem Sie die erste Gast ausführbare Datei hinzugefügt haben, klicken Sie mit der rechten Maustaste auf das Projekt für die Anwendung, und aktivieren Sie das **Hinzufügen neuer Dienst Fabric Service ->** das zweite ausführbare Gast-Projekt in der Lösung hinzufügen. Hinweis: Wenn Sie die Quelle in der Visual Studio-Projekt verknüpfen, wird die Visual Studio-Lösung erstellen sicherstellen, dass Ihrer Anwendungspaket mit den Änderungen in der Quelle auf dem neuesten Stand ist. 
...
Im nächsten Schritt erstellen Sie eine Anwendungspaket für die Anwendung Node.js. Im folgenden Code wird ein Fabric Service-Anwendung-Paket, die Anwendung Node.js enthält.
...
- **/ target** definiert das Verzeichnis, in dem das Paket erstellt werden soll. Dieses Verzeichnis muss aus dem Quellverzeichnis unterscheiden.
...
```
Die generierten ServiceManifest.xml verfügt jetzt über einen Abschnitt, der beschreibt wie der Webserver Node.js gestartet werden soll, wie im folgenden Codeausschnitt dargestellt:
...
```
> [AZURE.NOTE] Die Daten werden nicht im Fall eines Fehlers Knoten beibehalten werden, wenn Sie auf dem lokalen Verzeichnis des Knotens Datenverzeichnis MongoDB setzen. Sie sollten dauerhaften Speicher verwenden oder implementieren eine Replikatgruppe MongoDB um Datenverlust zu verhindern.  
