...
Wenn Ihr Clientprogramm Logik "Wiederholen" aufweist, können sie versuchen Sie, eine Verbindung nach der vorübergehende Fehlerstrukturanalyse-Time to alleine behoben zugewiesen wiederherzustellen.  Es empfiehlt sich, dass Sie für 5 Sekunden, bevor Sie Ihre erste "Wiederholen verzögern". Wiederholen nach einer kurzen Verzögerung kürzer als 5 Sekunden Risiken überfluten Cloud-Dienst. Bei jeder nachfolgenden Wiederholung die Verzögerung sollte wesentlich, wächst bis zu einem Maximum von 60 Sekunden.

Vorübergehende Fehlerstrukturanalyse-Fehler werden in der Regel als eine der folgenden Fehlermeldungen aus Ihrer Clientprogramme darstellen:

- Datenbank < Db_name > auf Server < Azure_instance > ist zurzeit nicht verfügbar. Wiederholen Sie die Verbindung zu einem späteren Zeitpunkt ein. Wenn das Problem weiterhin besteht, wenden Sie sich an Kundensupport und ermöglichen sie die Sitzung Tracing-ID < Nummer >

- Datenbank < Db_name > auf Server < Azure_instance > ist zurzeit nicht verfügbar. Wiederholen Sie die Verbindung zu einem späteren Zeitpunkt ein. Wenn das Problem weiterhin besteht, wenden Sie sich an Kundensupport, und teilen Sie sie die Sitzung Tracing-ID < Nummer >. (Microsoft SQL Server, Fehler: 40613)
...
|40501|20|Der Dienst ist ausgelastet. Wiederholen Sie die Anforderung nach 10 Sekunden ein. Vorfall ID: %ls. Code: %d.<br/><br/>*Hinweis:* Weitere Informationen hierzu finden Sie unter:<br/>• [Azure SQL-Datenbank Ressource Grenzwerte](sql-database-resource-limits.md).
|40613|17|Datenbank '%. & #x2a; ls' auf Server '% & #x2a; ls' ist zurzeit nicht verfügbar. Wiederholen Sie die Verbindung zu einem späteren Zeitpunkt ein. Wenn das Problem weiterhin besteht, wenden Sie sich an Kundensupport und ermöglichen sie die Sitzung Tracing ID des '% & #x2a; ls'.|
...
|40561|16|Fehler beim Kopieren der Datenbank. Die Quell-oder Zielliste nicht vorhanden ist.|
...
|40565|16|Fehler beim Kopieren der Datenbank. Nicht mehr als 1 gleichzeitige Datenbank kopieren aus derselben Quelle zulässig ist. Legen Sie die Zieldatenbank, und versuchen Sie es später erneut.|
|40566|16|Fehler bei der Datenbankkopie interner Fehler Legen Sie die Zieldatenbank, und versuchen Sie es erneut.|
...
|40569|16|Fehler beim Kopieren der Datenbank. Zieldatenbank weist mehr verfügbar. Legen Sie die Zieldatenbank, und versuchen Sie es erneut.|
...
## <a name="resource-governance-errors"></a>Ressource Governance Fehlern

Die folgenden Fehler werden durch übermäßige Verwendung von Ressourcen während der Arbeit mit Azure SQL-Datenbank verursacht. Zum Beispiel:
...
|40551|16|Die Sitzung wurde aufgrund übermäßiger beendet `TEMPDB` Verwendung. Versuchen Sie, Ihre Abfrage zum Verringern der Verwendung von Speicherplatz temporäre Tabelle ändern.<br/><br/>*Tipp:* Wenn Sie temporäre Objekte verwendet werden, zu sparen Speicherplatz in der `TEMPDB` Datenbank durch temporäre Objekte ablegen, nachdem sie von der Sitzung nicht mehr benötigt werden.|
...
|40553|16|Die Sitzung wurde aufgrund übermäßiger arbeitsspeicherauslastung beendet. Versuchen Sie, Ihre Abfrage zum Verarbeiten von weniger Zeilen zu ändern.<br/><br/>*Tipp:* Verringern die Anzahl der `ORDER BY` und `GROUP BY` Operationen in Transact-SQL-Code benötigt weniger Arbeitsspeicher der Abfrage.|
...
| :-- | :-- | :-- | :-- | :-- | :-- |
| 1132 | EX_RESOURCE | Die flexible Ressourcenpool hat Speichergrenzwert erreicht. Die Speicherplatzverwendung für den flexible Pool darf (%d) MB nicht überschreiten. | Flexible Ressourcenpool Grenzwert für Speicherplatz in MB. | Beim Versuch, Schreiben von Daten in einer Datenbank, wenn die Speichergrenze die flexible Ressourcenpool erreicht wurde. | Bitte sollten Sie die DTUs um Speichergrenzwert vergrößern, verringern Sie die Speicherung von einzelne Datenbanken innerhalb der flexible Ressourcenpool verwendet die flexible Ressourcenpool falls möglich erhöhen oder Entfernen von Datenbanken aus dem Pool flexible. |
...
| 40844 | EX_USER | Datenbank '%ls' auf dem Server '' %ls '' '' %ls '' Edition-Datenbank in eine flexible Ressourcenpool und kann keine Geschäftsbeziehung fortlaufender kopieren. | Datenbankname, Datenbankedition, -Servernamens | Ein StartDatabaseCopy Befehl für eine nicht-Premium Db in einem flexible Pool ausgegeben. | Demnächst |
| 40857 | EX_USER | Flexible Ressourcenpool für Server wurde nicht gefunden: "%ls", flexible Ressourcenpool Namen: "%ls". | Namen der Server; Flexible Ressourcenpool Namen | Angegebene flexible Pool ist auf dem angegebenen Server nicht vorhanden. | Geben Sie einen gültigen flexible Pool-Namen aus. |
...
| 40859 | EX_USER | Flexible Ressourcenpool unterstützt Dienstebene '%ls' nicht. | Flexible Ressourcenpool Dienstebene | Angegebenen Dienstebene ist nicht für die Bereitstellung von flexible Ressourcenpool unterstützt. | Geben Sie die richtige Edition oder nichts Dienstebene die Ebene der Standard-Dienst verwenden. |
| 40860 | EX_USER | Flexible Ressourcenpool '%ls' und Dienst Ziel '%ls' Kombination ist ungültig. | Flexible Ressourcenpool Name; Dienst Ebene Ziel Namen | Flexible Ressourcenpool und Dienst Ziel zusammen nur, wenn Dienst Ziel als 'ElasticPool' angegeben wird angegeben werden kann. | Geben Sie die richtige Kombination von flexible Ressourcenpool und Dienst Ziel. |
...
| 40865 | EX_USER | Die DTUs für den flexible Pool nicht überschreiten (%d) DTUs für Dienstebene ' % *!-Tabelle. | DTUs für flexible Ressourcenpool; Flexible Ressourcenpool Service Ebene. | Versuch der DTUs für den flexible Pool über das maximale Limit festzulegen. | Wiederholen Sie die Einstellung nicht größer als das maximale Limit der DTUs für den flexible Pool. |
| 40867 | EX_USER | Die max pro Datenbank DTU muss bei mindestens (%d) für den Dienstebene ' % *!-Tabelle. | DTU Max pro Datenbank; Flexible Ressourcenpool Dienstebene | Versuch der Max DTU pro Datenbank unter den unterstützten Grenzwert festzulegen. | Erwägen Sie die flexible Ressourcenpool Dienstebene, die die gewünschte Einstellung unterstützt. |
...
| 40870 | EX_USER | Die min DTU pro Datenbank kann nicht (%d) für den Dienstebene überschreiten ' % *!-Tabelle. | DTU min pro Datenbank; Flexible Ressourcenpool Dienstebene. | Versuch der DTU min pro Datenbank die unterstützten Beschränkung festzulegen. | Erwägen Sie die flexible Ressourcenpool Service Ebene, die die gewünschte Einstellung unterstützt. |
| 40873 | EX_USER | Die Anzahl der Datenbanken (%d) und DTU min pro Datenbank (%d) darf der DTUs die flexible Ressourcenpool (%d) nicht überschreiten. | Anzahl der Datenbanken in flexible Ressourcenpool; DTU min pro Datenbank; DTUs flexible Ressourcenpool. | Beim Versuch, DTU min für Datenbanken im flexible Pool angeben, die die DTUs der flexible Ressourcenpool überschreitet. | Sollten Sie die DTUs von flexible Speicherpool, erhöhen oder verringern Sie die min DTU pro Datenbank oder Verringern der Anzahl der Datenbanken im flexible Pool. |
...
| 40891 | EX_USER | Die min DTU pro Datenbank (%d) kann die Max DTU pro Datenbank (%d) nicht überschreiten. | DTU min pro Datenbank; DTU pro Datenbank max. | Bei dem Versuch, die min DTU pro Datenbank höher als die Max DTU pro Datenbank festlegen. | Stellen Sie sicher, dass die min DTU pro Datenbanken der Max DTU pro Datenbank nicht überschreiten. |
...
* [Erstellen Sie eine Datenbank flexible Ressourcenpool (c#)](sql-database-elastic-pool-create-csharp.md) 
...
* [Überwachen und Verwalten eines flexible Datenbank Ressourcenpool (PowerShell)](sql-database-elastic-pool-manage-powershell.md).
...
|18452|14|Fehler bei der Anmeldung. Der Benutzername aus einer nicht vertrauenswürdigen Domäne ist, und kann nicht verwendet werden, mit Windows authentication.%. & #x2a; ls (Windows Anmeldungen werden in dieser Version von SQL Server nicht unterstützt.)|
...
|40506|16|Angegebene SID ist für diese Version von SQL Server ungültig.|
|40507|16|' % & #x2a;!-Tabelle nicht mit den Parametern in dieser Version von SQL Server aufgerufen werden.|
|40508|16|USE-Anweisung wird nicht unterstützt, zum Umschalten zwischen Datenbanken. Verwenden Sie eine neue Verbindung für die Verbindung zu einer anderen Datenbank ein.|
...
|40512|16|Nicht mehr unterstützter Features "%ls" wird in dieser Version von SQL Server nicht unterstützt.|
...
|40515|16|Verweis auf Datenbank und/oder Server Name in '% & #x2a; ls' wird in dieser Version von SQL Server nicht unterstützt.|
