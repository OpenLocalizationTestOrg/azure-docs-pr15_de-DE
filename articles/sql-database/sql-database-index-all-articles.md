...
| 22 | [SQL Azure-Datenbank lernfähig &amp; anpasst](sql-database-learn-and-adapt.md) | Erfahren Sie, wie lernfähig und passt sich SQL-Datenbank |
...
| 45 | [Azure SQL-Datenbank Benchmark (Übersicht)](sql-database-benchmark-overview.md) | In diesem Thema werden der Azure SQL Datenbank Maßstab in Messen der Leistung des SQL Azure-Datenbank verwendet werden. |
...
| 47 | [Erste Schritte mit flexible Datenbanktools](sql-database-elastic-scale-get-started.md) | Grundlegende Erläuterung der Datenbank flexible Toolsfeature Azure SQL-Datenbank, einschließlich einfaches Beispiel-app ausgeführt. |
...
| 50 | [Verwenden von In-Memory OLTP (Preview) zum Verbessern der Leistung der Anwendung in SQL-Datenbank](sql-database-in-memory-oltp-migration.md) | Adopt In-Memory OLTP Transaktionen Leistung in einer vorhandenen SQL­Datenbank. |
...
| 54 | [Azure SQL-Datenbank und Leistung für einzelne Datenbanken](sql-database-performance-guidance.md) | In diesem Artikel helfen festzustellen, welche Dienstebene für eine Anwendung auswählen. Außerdem wird empfohlen, Methoden zum Optimieren Ihrer Anwendungs zu SQL Azure-Datenbank optimal zu nutzen. |
| 55 | [SQL Azure-Datenbank Abfrage Leistung Einblick](sql-database-query-performance.md) | Überwachen der Abfrage Leistung werden die meisten CPU-kompatiblen Abfragen für eine SQL Azure-Datenbank aufgeführt. |
| 56 | [Ändern Sie die Ebene und Leistung Dienstalter (Preisgestaltung Ebene) einer SQL-Datenbank](sql-database-scale-up.md) | Ändern der Dienstebene und Leistungsstufe einer SQL Azure-Datenbank zeigt, wie Sie die SQL-Datenbank nach oben oder unten skalieren. Ändern der Preisgestaltung Ebene einer SQL Azure-Datenbank. |
| 57 | [Überwachen der Leistung der Datenbank in Azure SQL-Datenbank](sql-database-single-database-monitor.md) | Lernen Sie die Optionen für Ihre Datenbank mit Azure Tools und dynamische Management Ansichten für die Überwachung aus. |
...
| &nbsp; | Titel | Beschreibung |
| --: | :-- | :-- |
| 60 | [Entwerfen von Mustern für mandantenfähigen SaaS-Anwendungen und Azure SQL-Datenbank](sql-database-design-patterns-multi-tenancy-saas-applications.md) | In diesem Artikel werden die Anforderungen und allgemeine Daten Architektur Mustern mandantenfähigen-Datenbank, die in einen Cloud-Umgebung ausgeführt Applications berücksichtigen müssen, und die verschiedenen vor-und Nachteile dieser Muster zugeordnet. Es wird erläutert, auch wie SQL Azure-Datenbank, mit deren flexible Pools und flexible Tools diese Anforderungen ohne Kompromisse Weise beheben. |
| 61 | [Migrieren Sie bestehende Datenbanken nach Skalierung](sql-database-elastic-convert-to-use-elastic-tools.md) | Konvertieren von sharded Datenbanken um flexible Datenbanktools durch Erstellen einer Shard Karte Manager verwenden |
...
| 92 | [Überwachen Sie und verwalten Sie einer Datenbank flexible Ressourcenpool mit Transact-SQL](sql-database-elastic-pool-manage-tsql.md) | Verwenden Sie T-SQL Azure SQL-Datenbank in eine flexible Ressourcenpool zu erstellen. Oder verwenden Sie T-SQL, um die Datenbank ein-und Pools verschieben. |
...
| 101 | [Übersicht über Geschäftskontinuität mit Azure SQL-Datenbank](sql-database-business-continuity.md) | Erfahren Sie, wie SQL Azure-Datenbank Geschäftskontinuität Cloud und Wiederherstellung der Datenbank unterstützt, und hilft beibehalten wichtiger Cloud-Anwendung, ausgeführt. |
...
| 108 | [Importieren einer BACPAC-Datei zum Erstellen einer SQL Azure-Datenbank](sql-database-import.md) | Erstellen Sie eine SQL Azure-Datenbank durch Importieren einer vorhandenen BACPAC-Datei ein. |
...
| 112 | [Wiederherstellen einer Azure SQL-Datenbank mithilfe von automatisierten Datenbanksicherungskopien](sql-database-recovery-using-backups.md) | Erfahren Sie in Zeitpunkt wiederherstellen, mit dem Sie die Anwendung einer SQL Azure-Datenbank zu einem vorherigen Punkt in (bis zu 35 Tage) zurückzusetzen kann. |
...
| &nbsp; | Titel | Beschreibung |
| --: | :-- | :-- |
| 134 | [Herstellen einer Verbindung mit SQL-Datenbank oder die Datawarehouse SQL Azure-Active Directory-Authentifizierung verwenden](sql-database-aad-authentication.md) | Erfahren Sie, wie mithilfe von Azure-Active Directory-Authentifizierung mit SQL-Datenbank zu verbinden. |
...
| 137 | [SQL-Datenbank - Clients älterer Versionen unterstützen und -IP-Endpunkt Änderungen für die Überwachung](sql-database-auditing-and-dynamic-data-masking-downlevel-clients.md) | Erfahren Sie mehr über SQL-Datenbank zur Unterstützung für kompatible Clients und IP-Endpunkt Änderungen für die Überwachung. |
| 138 | [Konfigurieren Sie mithilfe der PowerShell SQL Azure-Datenbank Server Ebene Firewall-Regeln](sql-database-configure-firewall-settings-powershell.md) | Informationen Sie zum Konfigurieren der Firewall für IP-Adressen, die auf SQL Azure-Datenbanken zugreifen. |
...
| 145 | [SQL-Datenbank-Authentifizierung und Autorisierung: Gewähren des Zugriffs](sql-database-manage-logins.md) | Erfahren Sie mehr über SQL-Datenbank Sicherheit Verwaltung speziell zum Verwalten der Datenbank Zugriff und Login Sicherheit über die wichtigsten Ebene Server-Konto. |
...
| 148 | [Wie häufig durchgeführte Verwaltungsaufgaben z. B. Zurücksetzen von Administratorkennworts in Azure SQL-Datenbank](sql-database-troubleshoot-permissions.md) | Beschreibt das allgemeine Verwaltungsaufgaben in SQL-Datenbank. Beispielsweise zurücksetzen Administratorkennworts, gewähren und Entfernen von Access aus. |
...
| 150 | [Konfigurieren einer Azure SQL-Datenbank Server Ebene Firewall-Regel mithilfe der Azure-Portal](sql-database-configure-firewall-settings.md) | Informationen Sie zum Konfigurieren der Firewall für IP-Adressen, die Azure SQL Server zugreifen. |
...
| 152 | [Verwalten von Azure Automatisierung Azure SQL-Datenbanken](sql-database-manage-automation.md) | Lernen Sie, wie die Automatisierung Azure Service zum Verwalten von SQL Azure-Datenbanken bei verwendet werden kann. |
...
| &nbsp; | Titel | Beschreibung |
| --: | :-- | :-- |
| 156 | [Ereignis Datei Zielcode für erweiterte Ereignisse in der SQL-Datenbank](sql-database-xevent-code-event-file.md) | Stellt PowerShell und Transact-SQL für eine Stichprobe von zwei Phasen Code, die das Ziel Ereignis-Datei in einem erweiterten Ereignis Azure SQL Datenbank veranschaulicht. Azure-Speicher ist Bestandteil dieses Szenario erforderlich. |
| 157 | [Anrufen Sie Puffer Zielcode für erweiterte Ereignisse in SQL-Datenbank](sql-database-xevent-code-ring-buffer.md) | Stellt eine Transact-SQL-Code Stichprobe, die durch die Verwendung des Ziels anrufen Puffer in SQL Azure-Datenbank, schnell und einfach besteht. |
| 158 | [Erweiterte Ereignisse in der SQL-Datenbank](sql-database-xevent-db-diff-from-svr.md) | Beschreibt erweiterte Ereignisse (XEvents) in Azure SQL-Datenbank, und wie Ereignis Sitzungen geringfügig Ereignis Sitzungen in Microsoft SQL Server variieren. |
...
| 165 | [Zum Verwalten der Sicherheit SQL Azure-Datenbank nach der Wiederherstellung](sql-database-geo-replication-security-config.md) | In diesem Thema wird erläutert, Sicherheitsaspekte zum Verwalten der Sicherheit nach einer Datenbank wiederherstellen oder ein Failover. |
| 166 | [Konfigurieren Sie Geo-Replikation für Azure-SQL­Datenbank mit Transact-SQL](sql-database-geo-replication-transact-sql.md) | Konfigurieren von Geo-Replikation für mit Transact-SQL Azure SQL-Datenbank |
...
| 173 | [Planen und Vorbereiten auf SQL-Datenbank V12 aktualisieren](sql-database-v12-plan-prepare-upgrade.md) | Beschreibt die Vorbereitungen und Einschränkungen beim Upgrade auf Version V12 von Azure SQL-Datenbank. |
...
| 175 | [Web und Business Edition Sonnenuntergangs häufig gestellte Fragen](sql-database-web-business-sunset-faq.md) | Finden Sie heraus, wenn die Azure SQL Web und Business-Datenbanken gelöscht werden werden, und erfahren Sie über die Features und Funktionen der neuen Service leisten mehr. |
...
| 180 | [Verwalten von Azure SQL-Datenbank mit SQL Server Management Studio](sql-database-manage-azure-ssms.md) | Erfahren Sie, wie Sie SQL Server Management Studio, um die Datenbank von SQL Server und Datenbanken verwalten. |
...
| 183 | [Verwenden von SQL Server Management Studio in Azure RemoteApp Verbindung mit SQL-Datenbank](sql-database-ssms-remoteapp.md) | Anhand dieses Lernprogramm erfahren Sie die Verwendung von SQL Server Management Studio Azure RemoteApp für Sicherheit und Leistung bei der Verbindung mit SQL-Datenbank |
...
| &nbsp; | Titel | Beschreibung |
| --: | :-- | :-- |
| 184 | [Datenverbindungsbibliotheken für SQL-Datenbank und SQL Server](sql-database-libraries.md) | Listet die kleinstmögliche Versionsnummer für jeden Treiber, die Verbindung mit SQL Azure-Datenbank oder mit Microsoft SQL Server-Clientprogramme verwenden können. Ein Link wird Version Informationen zu Treibern bereitgestellt, die nicht von Microsoft, sondern von der Community freigegeben werden. |
| 185 | [Beschränkungen der Ressource für Azure SQL-Datenbank](sql-database-resource-limits.md) | Diese Seite enthält einige allgemeine Ressource Grenzwerte für SQL Azure-Datenbank ein. |
| 186 | [Unterschiede der Azure SQL-Datenbank Transact-SQL](sql-database-transact-sql-information.md) | Transact-SQL-Anweisungen, die in Azure SQL-Datenbank weniger als vollständig unterstützt werden |
...
| &nbsp; | Titel | Beschreibung |
| --: | :-- | :-- |
| 187 | [Kopieren einer SQL Azure-Datenbank mithilfe der PowerShell](sql-database-copy-powershell.md) | Erstellen einer SQL Azure-Datenbank mithilfe der PowerShell kopieren |
| 188 | [Kopieren einer mit Transact-SQL SQL Azure-Datenbank](sql-database-copy-transact-sql.md) | Erstellen Sie Kopieren einer mit Transact-SQL Azure SQL-Datenbank |
...
| 190 | [SQL-Datenbank Preisgestaltung Ebene Empfehlungen](sql-database-service-tier-advisor.md) | Beim Ändern von Preise werden Ebenen in der Azure-Portal Preise Ebene Empfehlungen vorausgesetzt, sollten Sie die Ebene, der am besten geeignet für eine vorhandene Azure SQL-Datenbank die Arbeitsbelastung ausgeführt. Preisgestaltung Ebenen beschreiben die Dienstalter Ebene und Leistung von einer SQL-Datenbank. |
