<properties
   ...
   description="Informationen Sie zum Erstellen und Bereitstellen von NSGs in Azure Ressourcenmanager mithilfe von PowerShell"
...
# <a name="how-to-create-nsgs-in-resource-manager-by-using-powershell"></a>So erstellen Sie NSGs in Ressourcenmanager mithilfe von PowerShell
...
Führen Sie zum Erstellen einer benannten *NSG-Front-End-* basierend auf dem oben genannten Szenario NSG die folgenden Schritte aus:
...
4. Fügen Sie die oben in einer neuen NSG benannte **NSG-Front-End**erstellten Regeln hinzu.

        $nsg = New-AzureRmNetworkSecurityGroup -ResourceGroupName TestRG -Location westus
        -Name "NSG-FrontEnd" -SecurityRules $rule1,$rule2

5. Überprüfen Sie die Regeln in der NSG erstellt ein.

        $nsg

    Die Ausgabe nur für die Sicherheitsregeln mit:
...
6. Zuordnen der NSG über erstellt, mit dem *Front-End* -Subnetz an.
    ...
    >[AZURE.WARNING] Die Ausgabe für den Befehl oben zeigt den Inhalt für das virtuelle Netzwerk-Konfiguration-Objekt, das nur auf dem Computer vorhanden ist, in dem Sie PowerShell ausgeführt werden. Sie ausführen müssen die `Set-AzureRmVirtualNetwork` Cmdlet, diese Einstellungen in Azure zu speichern.

7. Speichern Sie die neuen VNet-Einstellungen in Azure.

        Set-AzureRmVirtualNetwork -VirtualNetwork $vnet

    Die Ausgabe mit den NSG Teil:
...
2. Erstellen einer Sicherheitsregel Zugriff auf das Internet blockieren.
...
4. Zuordnen der NSG über mit dem *Back-End-* Subnetz erstellt.

        Set-AzureRmVirtualNetworkSubnetConfig -VirtualNetwork $vnet -Name BackEnd
            -AddressPrefix 192.168.2.0/24 -NetworkSecurityGroup $nsg

    Ausgeben Sie nur die *Back-End-* Subnetz Einstellungen mit, beachten Sie den Wert für die Eigenschaft **NetworkSecurityGroup** :
...
## <a name="how-to-remove-an-nsg"></a>Zum Entfernen einer NSG

Führen Sie die folgenden Schritte aus, um eine vorhandene NSG, *NSG-Front-End* in diesem Fall bezeichnet löschen:

Führen Sie die nachfolgend aufgeführten **Entfernen-AzureRmNetworkSecurityGroup** , und achten Sie darauf, dass Sie die Ressourcengruppe enthalten sein, der die NSG befindet.
