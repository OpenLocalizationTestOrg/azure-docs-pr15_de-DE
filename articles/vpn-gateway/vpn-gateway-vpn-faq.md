...
Ja. Es gibt sogar keine Region-Einschränkung. Eine virtuelles Netzwerk kann mit einem anderen virtuellen Netzwerk in der gleichen Region oder in ein anderes Azure Region verbinden.

### <a name="can-i-connect-virtual-networks-in-different-subscriptions"></a>Kann ich eine virtuelle Netzwerke in anderen Abonnements herstellen?
Ja.

### <a name="can-i-connect-to-multiple-sites-from-a-single-virtual-network"></a>Kann ich eine Verbindung zu mehreren Websites aus einem einzigen virtuellen Netzwerk herstellen?
...
- [VNet-zu-VNet](virtual-networks-configure-vnet-to-vnet-connection.md) – dieser Art von Verbindung ist eine Website-zu-Standort-Konfiguration identisch. VNet zu VNet ist eine VPN-Verbindung über IPsec (IKE Version 1 und 2 IKE). Es sind kein Gerät VPN erforderlich.
...
**Punkt-zu-Standort** ermöglichen die von einem einzigen Computer von überall eine Verbindung Verbindungen auf einen anderen Wert befindet sich in Ihrem Netzwerk virtuelle. Es wird den Windows-Box-VPN-Client verwendet. Als Teil der Punkt-zu-Standort-Konfiguration installieren Sie ein Zertifikat und ein VPN-Client-Konfigurationspaket, die Einstellungen enthält, die Ihrem Computer für die Verbindung zu einem beliebigen virtuellen Computern oder Rolleninstanz innerhalb des virtuellen Netzwerks erlauben. Es ist hervorragend, wenn Sie ein virtuelles Netzwerk eine Verbindung herstellen möchten, aber am lokalen nicht zur Verfügung. Es ist auch eine gute Wahl, wenn Sie keine Zugriff auf VPN Hardware oder eine extern ausgerichteten IPv4-Adresse, die für eine Verbindung zwischen Standorten erforderlich sind. 
...
ExpressRoute Verbindungen bieten eine höhere Sicherheit, mehr Zuverlässigkeit, höhere Bandbreite und unteren Wartezeiten als normalen Verbindungen über das Internet. In einigen Fällen kann mithilfe von ExpressRoute-Verbindungen zum Übertragen von Daten zwischen Ihrem lokalen Netzwerk und Azure auch signifikante Kostenvorteile erzielt werden. Wenn Sie eine Cross lokale Verbindung aus Ihrem lokalen Netzwerk in Azure bereits erstellt haben, können Sie auf eine Verbindung ExpressRoute Beibehaltung Netzwerks virtuelle intakt migrieren.
...
Dies ist die erwartete Verhalten für Policy-basierte (auch bekannt als statisches routing) VPN-Gateways. Wenn Sie der Datenverkehr über den Tunnel für mehr als 5 Minuten im Leerlauf befindet, wird der Tunnel nach unten abgebrochen werden. Beim Starten von Datenverkehr in Richtung parallelen wird der Tunnel sofort wiederhergestellt.
...
Andere Software VPN Lösungen sollte mit unseren Gateway funktionieren, solange sie Industry standard IPSec-Implementierungen entsprechen. Wenden Sie sich an den Hersteller der Software, Konfiguration und Support-Anweisungen.
...
### <a name="can-i-use-any-software-vpn-client-for-point-to-site-that-supports-sstp"></a>Kann ich alle Software VPN-Client für Punkt-zu-Standort verwenden, die die SSTP unterstützt?

Nein. Support ist nur für die oben aufgeführten Windows-Betriebssystem-Versionen beschränkt.
...
### <a name="what-is-a-route-based-dynamic-routing-gateway"></a>Was ist ein Gateway Routing-basierten (Dynamic routing)?
...
### <a name="can-i-get-my-vpn-gateway-ip-address-before-i-create-it"></a>Kann ich meine VPN Gateway IP-Adresse erhalten, bevor ich es erstellen?
...
Azure VPN verwendet Authentifizierung PSK (vorinstallierter Schlüssel). Wir generieren einen vorinstallierten Schlüssel (PSK) aus, wenn wir den VPN-Tunnel erstellen. Sie können die automatisch generierte PSK eigene mit dem Festlegen vorinstallierter Schlüssel PowerShell-Cmdlet oder REST-API ändern.

### <a name="can-i-use-the-set-pre-shared-key-api-to-configure-my-policy-based-static-routing-gateway-vpn"></a>Können die festlegen vorinstallierter Schlüssel API so konfigurieren Sie meine Policy-basierte verwenden (statisches routing) Gateway VPN?

Ja, kann das Festlegen vorinstallierter Schlüssel API und PowerShell-Cmdlet Azure Policy-basierte (statisch) VPN und Routing-basierten (dynamic) Weiterleitung VPN konfigurieren verwendet werden.
...
## <a name="multi-site-and-vnet-to-vnet-connectivity"></a>Mit mehreren Standorten und VNet-VNet-Konnektivität
...
Nein, beide virtuelle Netzwerke müssen Routing-basierten (dynamisches routing) verwenden VPN.

### <a name="is-the-vnet-to-vnet-traffic-secure"></a>Ist der VNet-VNet-Datenverkehr sicher?

Ja, wird es von IPSec-/IKE-Verschlüsselung geschützt.
...
### <a name="can-i-use-point-to-site-vpns-with-my-virtual-network-with-multiple-vpn-tunnels"></a>Kann ich verwenden Punkt-zu-Standort VPN mit meinem virtuelle Netzwerk mit mehreren VPN-Tunnel?
...
Nein. Überlappende Adressbereiche verursacht im Netzwerk Konfiguration Datei hochladen oder "Erstellen von virtuellen Netzwerk" zum fehlschlagen.
...
Nein, alle VPN-Tunnel, einschließlich Punkt-zu-Standort VPN, freigeben, demselben Gateway Azure VPN und die Bandbreite zur Verfügung.

### <a name="can-i-use-azure-vpn-gateway-to-transit-traffic-between-my-on-premises-sites-or-to-another-virtual-network"></a>Kann ich Azure VPN-Gateway anzuwenden ist Datenverkehr zwischen meinem lokalen Websites oder einem anderen virtuelle Netzwerk verwenden?
...
### <a name="if-my-virtual-machine-is-in-a-virtual-network-and-i-have-a-cross-premises-connection-how-should-i-connect-to-the-vm"></a>Wenn meine virtuellen Computern befindet sich in einem virtuellen Netzwerk, und ich verfüge über einen Cross lokale Verbindung, wie sollte ich mit dem virtuellen Computer verbinden?

Es gibt einige Optionen. Wenn Sie RDP aktiviert haben, und einen Endpunkt erstellt haben, können Sie Ihre virtuellen Computern mithilfe der VIP verbinden. In diesem Fall würden Sie angeben der VIP und den Port, dem Sie in eine Verbindung herstellen möchten. Sie müssen den Port auf Ihre virtuellen Computern für den Datenverkehr zu konfigurieren. Sie möchten in der Regel, wechseln Sie zum klassischen Azure-Portal und die Einstellungen für RDP-Verbindung mit Ihrem Computer speichern. Die Einstellungen enthalten die erforderlichen Verbindungsinformationen.

Wenn Sie ein virtuelles Netzwerk mit Cross lokale Verbindung konfiguriert haben, können Sie Ihre virtuellen Computern mithilfe des internen DIP oder private IP-Adresse verbinden. Sie können auch mit Ihrer virtuellen Computern durch internen DIP aus einem anderen virtuellen Computern herstellen, die auf dem gleichen virtuellen Netzwerk befindet. Sie können nicht RDP zu Ihrem virtuellen Computern mithilfe von DIP aus, wenn Sie von einem Speicherort außerhalb des virtuellen Netzwerks verbinden. Beispielsweise können nicht, wenn Sie ein virtuelles Punkt-zu-Standort-Netzwerk konfiguriert haben, und Sie keine Verbindung, von Ihrem Computer herstellen, Sie virtuellen Computer nach DIP verbinden.
